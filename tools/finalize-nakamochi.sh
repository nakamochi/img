...
run_main()
{
    ...
    patch_bitcoind_conf "$USD_MOUNT_POINT"/home/bitcoind/mainnet.conf "$bitcoind_rpcauth" "$bitcoind_onion_hostname"
    echo "done."

    # modify lnd configuration
    echo -n "Finalizing lnd configuration ..."
    lnd_onion_hostname="$(cat "$SSD_MOUNT_POINT"/tor/lnd/hostname)"
    patch_lnd_conf "$USD_MOUNT_POINT"/home/lnd/lnd.mainnet.conf "$bitcoind_rpcuser" "$bitcoind_rpcpass" "$lnd_onion_hostname"
    echo "done."
